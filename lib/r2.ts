...
function buildObjectUrl(config: R2Config, key: string, search?: string) {
  const encodedKey = key
    .split('/')
    .map((segment) => encodeURIComponent(segment))
    .join('/');

  return `${config.baseEndpoint}/${encodedKey}${search ? `?${search}` : ''}`;
}
  ...
  async function deleteNextObject() {
    while (nextIndex < uniqueKeys.length) {
      ...
      try {
        ...
        const signature = createSignature(activeConfig, date, canonicalRequest);
